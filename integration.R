...
# need to activate the conda py3 environment 
##----------------------------------------------------------------------------------------
...
getClusterMarkers <- function(qcProcessedResults, integrationMethod = 'CCA', nfeatures = 2000, resDirName = NULL, topN = 10, ribo = F, int.k.weight = 100) {
  ...
  ## 4.3 umap, tsne, knn/clusters | funing
  ...
  seuratObjFinal                <- FindClusters(seuratObjFinal, 
                                                algorithm = 4,
                                                method = "igraph",
                                                resolution = 0.5)
...
metadata <- read.csv("metadata_integration_2.txt", sep='\t')
...
qcRes <- processQC(metadata = metadata, 
                   resDirName = 'integration_2_leiden', 
                   genomeSpecies = 'human', 
                   minCells = 3, # defualt
                   mtFiltering = T, 
                   minFeatures = 100, 
...
IntegratedRes <- getClusterMarkers(qcRes,
                                   integrationMethod = "RPCA",
                                   int.k.weight = 100, # specify the number of neighbors to consider when weighting anchors in the step of integration
                                   nfeatures = 1000,
                                   topN = 50)

seuratObj1 <- readRDS(paste0("integration_2_leiden/RDS_Dir/integration_2_leiden.rds"))
seuratObj1@assays$RNA <- JoinLayers(seuratObj1@assays$RNA)

saveRDS(seuratObj1, file = paste0("integration_2_leiden/RDS_Dir/integration_2_leiden.rds"))
